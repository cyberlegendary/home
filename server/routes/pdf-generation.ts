...
import { getSignaturePosition } from "../config/signaturePositions";
...
async function generateClearancePDF(submission: any): Promise<Uint8Array> {
  ...
  if (signatureData) {
    try {
      ...
      const sigPosition = getSignaturePosition("clearance-certificate-form");
...
async function generateDiscoveryPDF(submission: any): Promise<Uint8Array> {
  ...
  if (signatureData) {
    try {
      ...
      const sigPosition = getSignaturePosition("discovery-form");

      page.drawImage(img, {
        x: sigPosition.x,
        y: sigPosition.y,
        width: Math.min(dims.width, sigPosition.width),
        height: Math.min(dims.height, sigPosition.height),
        opacity: sigPosition.opacity || 0.7, // Make signature see-through
      });
...
async function generateLiabilityPDF(submission: any): Promise<Uint8Array> {
  ...
  if (signatureData) {
    try {
      ...
      const sigPosition = getSignaturePosition("liability-form");
